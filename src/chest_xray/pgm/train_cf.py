...
from utils_pgm import update_stats, plot_cf
...
from mimic import preprocess_mimic
...
def get_metrics(preds, targets):
    ...
    for k in preds.keys():
        if k == "age":
            ...
            stats[k + "_mae"] = torch.mean(torch.abs(targets[k] - preds_k)).item()
    return stats
    ...
    for i, batch in loader:
        ...
        if i % args.plot_freq == 0:
            ...
            save_path = os.path.join(
                args.save_dir, f"{args.step}_{split}_{do_k}_cfs.png"
            )
            save_plot(
                save_path,
                batch,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pgm_path", help="path to load pgm checkpoint.", type=str, default=""
    )
    parser.add_argument(
        "--predictor_path",
    ...
    parser.add_argument(
        "--vae_path", help="path to load vae checkpoint.", type=str, default=""
    )
